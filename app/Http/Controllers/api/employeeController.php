...
class employeeController extends Controller
{
    ...
    public function getemployee(Request $request){
            ...
            $query->where(DB::raw("CONCAT(`name`, ' ', `display_emp_id`)"), 'LIKE', "%" . $request->searchTerm . "%");
    ...
    public function createEmployee(addEmployeeValidate $request){
        ...
        $next_id = employee::withoutGlobalScopes()->max('emp_id') + 1;
        $display_emp_id = employee::max('display_emp_id') + 1;
        $request->request->add(['emp_id' => $next_id]);
        $request->request->add(['display_emp_id' => $display_emp_id]);
