...
class UserController extends Controller
{
   ...
   public function addUser(Request $request){
        ...
        $validate = Validator::make($request->all(), [ 
            ...
            'password' => 'required', 
            'mobile' => 'max:11', 
            'role' => IsSuperAdmin(\JWTAuth::user()) ? '' :'required', 
        ...
        $input['role'] = IsSuperAdmin(\JWTAuth::user()) ? CompanyAdminRoleRef() : $input['role'];
    ...
    public function updateUser(Request $request){
        ...
        $error = false;
        if(IsSuperAdmin(\JWTAuth::user()) || $user->role == CompanyAdminRoleRef())
        {
    ...
    public function getUsers(Request  $request){ 
        ...
        if ($request->searchTerm != '' && $request->searchTerm) {
            $query = User::query();
            $query->where(DB::raw("CONCAT(`id`, ' ', `email`, ' ', `name`)"), 'LIKE', "%" . $request->searchTerm . "%");
            $query->when(IsSuperAdmin(\JWTAuth::user()) , function ($q) {
                            return $q->where('role', CompanyAdminRoleRef());
                        });
            $query->orderBy('id','desc');
            $resourceRecords = $query->get();
        }
        else
        {   
            $query = User::query();
    ...
    public function get_permissions(User  $user){ 
            ...
            $tmpPermissions =  $user->userLevelUnscoped->permissions;

            $tmpPermissions = $tmpPermissions->toArray();
            //dd (IsSuperAdmin(\JWTAuth::user()));
           // return $tmp;
            $permissions = [];
            if(IsSuperAdmin(\JWTAuth::user()))
            {
                ...
                foreach (explode(',','C,D,E,V') as $childKey => $singlePermission) {
